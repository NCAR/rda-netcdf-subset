...
import argparse
import datetime
import xarray # takes a few seconds to load. Improve by only importing what you need
import logging
import numpy
...
def open_file(filename):
    ...
    if is_remote(filename):
        logging.debug('remote file '+ filename)
...
def spatial_subset(ds, nlat=None, slat=None, elon=None, wlon=None, **args):
    ...
    if elon > 360 and wlon < 360:
        lon_mask = (ds[lon] <= float(elon)%360) | (ds[lon] >= float(wlon)%360)
    else:
        lon_mask = (ds[lon] <= float(elon)%360) & (ds[lon] >= float(wlon)%360)
